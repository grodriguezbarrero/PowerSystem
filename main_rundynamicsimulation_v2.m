        ...
        for igenonline = 1:ngenonline
            ...
            %if igenonline == 1 % for the FIRST GENERATOR DOWN in each scenario
                for nWGgroupsonline = 0:3
                    ...
                    pdem = pdem_CG + nWGgroupsonline*3*pinitwindgen*1.5;  % total demand in MW
                    v_pshed0MW = v_pshed0/100*pdem;                    
    ...
    for i_scenario = 1:num_scenarios            % iterate through each scenario
        for j_simulation = 1:num_gen_in_total   % iterate through every generator
            ...
            if m_genscenarios(i_scenario, j_simulation) ~= 0
                ...
                title(['Frequency with ',  num2str(j_simulation+10), ' shut off']);
                ...
                title(['Power shedded by UFLS with ',  num2str(j_simulation+10), ' shut off']);
                ...
                title(['WG penetration with ',  num2str(j_simulation+10), ' shut off']);
                legend('Zero WG', '3 WGs', '6 WGs', '9 WGs');
                xlabel('Time (s)')
                ylabel('WG penetration (%)')
                hold off;
                
                sgt = sgtitle(['Scenario ', num2str(i_scenario)],'Color','red');
                sgt.FontSize = 18;
            end
        end
    end
...
%                 ha2 = hf2.CurrentAxes;
%                 for sim_in_scenario_wg = 1:4   % four sub-scenarios: 0, 1, 2, and 3 WGs
%                     plot(ha2,c_t_wg{isim_wg},fbase+c_w_wg{isim_wg}*fbase,'Color',v_colours(sim_in_scenario_wg));hold on;
%                     isim_wg = isim_wg - 1;
%                 end
...
%                 legend('Zero WG', '3 WG', '6 WGs', '9 WGs');
%                 xlabel(ha2,'Time (s)')
%                 ylabel(ha2,'Total power shedded by UFLS (pu)')
%                 hold off;
% 
%                 isim_wg = isim_wg + 4;
% 
